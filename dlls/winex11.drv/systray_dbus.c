        ...
        void *user_data )
{
    ...
    struct tray_icon *icon;
    const int tool_tip_len=4*TOOL_TIP_LEN;
    char utf8_tool_tip[TOOL_TIP_LEN];
    int i;
    ...
    if (strcmp(mpath, "/StatusNotifierItem") == 0)
    {
        ...
        if (p_dbus_message_is_method_call(msg, "org.freedesktop.DBus.Properties", "Get") )
        {
            ...
            if(strcmp(req_prop, "Category") == 0) {
                ...
                if((i = wine_utf8_wcstombs(0, icon->tiptext, strlenW(icon->tiptext)+1, utf8_tool_tip, tool_tip_len)) < 0)
                {
                    ERR( "DBus: UTF8 tool tip string conversion failed, code: %d\n", i );
                    error_reply_failed(msg);
                    return ret;
                }
                reply = create_reply_propget_s(msg, utf8_tool_tip);
                ...
                if (icon->icon_data && (icon->icon_w > 0) && (icon->icon_h > 0))
                {
                    ...
                    reply = create_reply_propget_tooltip(msg,
                        ...
                        utf8_tool_tip, /* tip title (szTip member of NOTIFYICONDATA) */
                        "");           /* tip text, empty */
                }
                else
                {
        ...
        else if (p_dbus_message_is_method_call(msg, "org.freedesktop.DBus.Properties", "GetAll"))
        {
            ...
            reply_propgetall_append_s(&arr_iter, "Title", utf8_tool_tip);
            ...
            if (icon->icon_data && (icon->icon_w > 0) && (icon->icon_h > 0))
            {
                ...
                reply_propgetall_append_tooltip(
                    ...
                    utf8_tool_tip, /* tooltip title (this is szTip member if NOTIFYICONDATA) */
                    "" /* tooltip text, empty */
                );
            }
            else
